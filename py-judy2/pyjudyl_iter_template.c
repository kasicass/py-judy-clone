...
<P>_itervalues_to(PyObject *container, PyObject *key)
{
  ...
  PY_TO_INDEX;
  return <P>_iter_new((<CT> *)container, "itervalues_to",
		      START, index, 1, &<P>IterValue_Type);
...
{
  Word_t start, end;
  if (!<P>_parse_range(args, "itervalues_range", &start, &end))
    return NULL;
  return <P>_iter_new((<CT> *)container, "itervalues_range",
		      start, end, 1, &<P>IterValue_Type);
...
{
  Word_t start, end;
  if (!<P>_parse_range(args, "iteritems_range", &start, &end))
    return NULL;
  return <P>_iter_new((<CT> *)container, "iteritems_range",
		      start, end, 1, &<P>IterItem_Type);
